...
4. **Launcher completion ordering.** Define the exact order for progress, terminal result, accepted-snapshot synchronization, selected-modpack reapplication, launch-state recalculation, semantic completion, and the presenter’s final notification.
...
1. Complete the owner’s test-expectation edit for the approved `Home` label; retain the established 195-test baseline.
