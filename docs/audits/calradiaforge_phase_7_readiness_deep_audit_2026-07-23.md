...
| Mod pipeline | `ModPipelineManager` owns scan/install admission, stop/cancel, quiescence, accepted snapshot publication, and complete-scan cache authorization. | The manager—not a new coordinator—remains the exclusive long-running mod-work boundary. |
