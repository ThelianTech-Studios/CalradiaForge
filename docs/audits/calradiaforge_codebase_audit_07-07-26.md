...
| Cleanup helpers lack containment checks | `ModExtractor.CleanupTempDirectory`, `ModpackData.DeleteModpack` | Medium | Reusable delete helpers can delete based on caller-provided paths. | Resolve full path and verify intended root before deleting. |
| URL launcher accepts any scheme | `ExplorerHelper.OpenUrl` | Low | Safe today for hard-coded URLs; risky if future user/Nexus URLs are passed. | Allow only `https` for external web links; route `nxm` through Nexus boundary. |
| No Nexus credential code to audit | `CalradiaForge.Nexus` | Cleanup | DPAPI/SSO/download behavior is planned only. | Implement only inside Nexus with DPAPI CurrentUser and no secret-bearing logs. |

No stored credentials, API keys, or Nexus tokens were found in source or config code. `AppConfig` currently stores app settings only, and the docs correctly forbid secrets there.

## 10. Data, Persistence, Cache, and Installer Workflow Audit

| Workflow / Data Area | Current State | Gap / Risk | Recommendation |
|---|---|---|---|
| App config | JSON key/value store via `AppConfig`, typed facade via `AppConfigSettings` | Direct writes; raw debug values | Atomic writes, corruption recovery, redaction/secret denylist. |
| Mod cache | `ModsData` current/backup data files | Backup exists but writes are not atomic; corrupt JSON handling is limited | Atomic write + backup restore fallback. |
| Modpack storage | JSON files under app root modpacks directory | Good helper split, but direct export write in service | Route all writes through data helper and validate import schema. |
| Last-used load order | Separate `last_used_mods.data` | Direct write | Same atomic persistence strategy. |
| Archive lifecycle | Extracts to app-managed hidden `Extraction` directory with GUID subfolders | Needs entry containment and size/file-count policy | Add archive preflight, limits, and cleanup recovery. |
| Installer workflow | Sequential queue, progress events, cancellation token | Overwrite/delete lacks backup/reserved guard | Add overwrite policy and module identity validation. |
| BLSE workflow | Special-case install path inside Core pipeline | Broad trust and overwrite into game bin | Add allowlist/manifest, backups, and tests. |
| Nexus download/cache | Directories exist in `AppPaths`; runtime not implemented | Ownership and metadata lifecycle still planned | Decide whether Core owns paths only or Nexus owns cache service. |
| Schema/versioning | Models serialize directly; no schema version | Future migrations may be fragile | Add `schemaVersion` to persisted modpack/cache metadata before broad changes. |

## 11. UI / UX / WPF Audit

| UI Area | Current State | Issue | Recommendation |
|---|---|---|---|
| Page structure | WPF pages own bindings and event handlers | Large page code-behind doubles as viewmodel | Extract viewmodels incrementally. |
| Status/progress visibility | Properties such as `DependencyWarningText`, `StatusText`, `IsRefreshing`, `IsInstalling` are updated | `rg` found updates but no obvious XAML bindings for several status fields | Bind status/busy/empty states or remove dead state. |
| Toasts | Toast service/viewmodels are comparatively structured | Some workflow errors may be toast-only | Pair toasts with persistent page status where relevant. |
| Async UI dispatch | Installer progress dispatches to UI thread | Completion uses `async void` and nested async dispatcher lambda | Make completion task observed and reset state in `finally`. |
| Navigation | Main window navigation and page refresh guards exist | Startup/navigation refresh races have required past fixes | Add regression tests around service state, or simplify with viewmodels. |
| Accessibility | Tooltips exist; no `AutomationProperties` found | Icon-only controls and disabled focus visuals reduce accessibility | Add automation names, keyboard focus visibility, and accessible labels. |
| Localization | XAML bindings to translation service are strong | Many C# strings and tooltips remain hardcoded | Audit user-facing strings and localize high-traffic workflows. |
| Theme/resources | Resource dictionaries are organized by page/system | Large resource files but acceptable | Keep; consider design-system doc before major UI expansion. |

## 12. Testing and QA Readiness

There is no test project checked in today. `dotnet test source\CalradiaForge.slnx` completes but does not run tests.

| Test Area | Current Coverage | Missing Coverage | Priority |
|---|---|---|---:|
| Build validation | Manual local build only | CI build on Windows with .NET 10 | Development-blocking for release confidence |
| Parser tests | None | `SubModule.xml` parsing variants and malformed XML | Important before feature expansion |
| Archive extraction tests | None | Traversal, nested root, corrupt archive, unsupported type, size/file count | Development-blocking before Nexus downloads |
| Installer tests | None | Reserved folders, overwrite/backup, cancellation, BLSE detection/copy | Development-blocking before Nexus downloads |
| Config/cache tests | None | Atomic writes, corrupt JSON, backup restore | Important before feature expansion |
| Modpack tests | None | Sanitization, import/export, last-used, missing mods | Important before feature expansion |
| Launch tests | None | Argument construction, Steam/Epic/GamePass validation paths | Regression safety |
| UI tests | None | Binding visibility, command enablement, navigation refresh | Later polish after viewmodel extraction |
| Nexus tests | None | Auth storage, downloader, queue, NXM handling | Required when Nexus implementation begins |

## 13. Dependency and Package Audit

| Dependency / Package Area | Current State | Concern | Recommendation |
|---|---|---|---|
| `SevenZipWrapper 0.9.5-beta` | Core dependency for archive extraction | Beta package, AMD64 assembly mismatch with MSIL projects, Windows-only analyzer warnings | Decide x64-only target or replacement/packaging strategy; add archive safety tests. |
| `Newtonsoft.Json 13.0.4` | Used in Core and Nexus | Fine, but JSON persistence lacks schemas/atomicity | Keep or eventually centralize serialization settings. |
| `MahApps.Metro 2.4.11` | UI styling | Stable but version strategy undocumented | Track updates before public release. |
| `MahApps.Metro.IconPacks.Material 6.2.1` | UI icons | No issue found | Keep. |
| `gong-wpf-dragdrop 4.0.0` | Mods drag/drop | Appropriate for WPF drag/drop | Keep; add UI/manual regression checklist. |
| Package management | Per-project package refs | No central version management or lock file | Consider `Directory.Packages.props` and lock file after project stabilizes. |
| SDK | No `global.json` | .NET 10 SDK can float across machines | Add `global.json` when release build SDK is chosen. |
| Target framework alignment | UI Windows-specific; Core/Nexus `net10.0` | Core uses Windows-only APIs but target is cross-platform | Document or target Windows where appropriate. |

## 14. Documentation Audit and Future Development Direction

### 14.1 Current Documentation State

Strong documentation:
- `AGENTS.md` clearly defines architecture rules, forbidden changes, Nexus rules, and build/test commands.
- `docs/Architecture/Solution/ARCHITECTURE.md` accurately describes the layered structure.
- Project docs under `docs/Architecture/Projects/` are useful and mostly aligned with source.
- System docs for config, logging, mod management, modpacks, launcher, localization, and EULA provide good source-of-truth boundaries.
- `docs/Architecture/Nexus/NEXUS_INTEGRATION_ARCHITECTURE.md` is detailed and correctly marks Nexus as planned/locked rather than implemented.

Stale or conflicting documentation:
- `docs/CHANGELOG.md` reaches `0.13.1`, while `CalradiaForge.UI.csproj` and `CalradiaForge.Core.csproj` still declare `0.12.15`.
- `README.md` links mix `ThelianTech` and `ThelianTech-Studios`, uses `master_docs`, and references `CONTRIBUTINGS.md` while the repo has `docs/CONTRIBUTIONS.md`.
- `docs/TODO_v1.md` says NexusModsAPI will be implemented for the 1.0 release, while architecture docs describe Nexus runtime as deferred/planned and owner decisions remain open.
- `AppPaths` comments still mention LocalAppData for downloads/extraction although code and changelog now resolve under app root.
- Changelog references a retained test scaffold, but no test project is checked in.

Planning-only documentation:
- Nexus auth, downloads, metadata, NXM handler, update checking, queue, partial downloads, and DPAPI credential flow.
- Epic/GamePass launch backend.
- Post-v1 roadmap and LTS plan.
- Settings WIP tab.

Missing documentation:
- Testing strategy.
- UI/WPF architecture pattern and viewmodel/command policy.
- Async/threading policy for service-owned workflows.
- Security model for archive trust, install overwrite, logging redaction, and Nexus credentials.
