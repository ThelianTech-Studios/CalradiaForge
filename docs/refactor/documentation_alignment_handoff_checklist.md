...
| Phase 5 | Game-platform detection/workflow, Steam split-library behavior, manual configuration, startup notifications, scanner safety, test migration, and the completion audit are accepted only after implementation and verification. | Platform/path detection, configuration, launcher, mod-management, testing/QA, and relevant architecture docs. | Planned; [detailed Phase 5 plan](game_platform_detection_and_path_workflow_plan.md) is the sole detailed authority. | Yes, including owner Steam split-library smoke evidence. |
| Phase 6.A | One logger owner, startup cleanup, shutdown quiescence, exact-once close, active-handle release, collision-safe archive, and provider disposal order are accepted only after implementation review. | Application lifecycle/startup/shutdown, logging architecture, security/trust-boundary, configuration/retention, and testing/QA docs. | Planning gate; unresolved options are not accepted architecture. | Yes. |
| Phase 6.B | Legacy caller migration includes review that credential-owning components do not intentionally pass credentials or authentication material to logs. | Logging architecture, security/trust-boundary, testing/QA, and relevant Nexus docs. | Planning artifact until caller migration is implemented and verified. | Yes, for future credential/export policy. |
