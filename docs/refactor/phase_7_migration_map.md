...
|---|---|---|---|---|
| `ModsPage.xaml` and `.xaml.cs` -> `LauncherPage.xaml` and `.xaml.cs` | Retained DI `ModsPage`; `MainWindow` navigation and `Nav_ModsTab` use Mods identity. | Behavior-preserving `LauncherPage` and **Launcher** label; update CLR/XAML, navigation, localization, tests, comments, and documentation. Do not rename mod-domain types. | Clean XAML/DI/navigation build; localization and rename-boundary tests; stale-reference inventory. | Reserve `ModsPage` for future mod management; Phase 8 uses `LauncherView`/`LauncherViewModel`. |
